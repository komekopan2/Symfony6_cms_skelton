...
class EnvCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // version
        $q = new Question("Engine version? [mariadb-10.5.15]: ", "mariadb-10.5.15");
        $q->setAutocompleterValues(["mariadb-10.5.15", "5.6"]);
        ...
        if(!$qHelper->ask($input, $output, $q)) {
            ...
            $password = $qHelper->ask($input, $output, $q);
            $q = new Question("SMTP host? [af118.secure.ne.jp]: ", "af118.secure.ne.jp");
            $host = $qHelper->ask($input, $output, $q);
